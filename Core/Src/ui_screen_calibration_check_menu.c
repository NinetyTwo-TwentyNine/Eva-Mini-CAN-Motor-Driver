...
static uint8_t label_tab_ids[CCM_ELEMENT_TEXT_COUNT] = { 1, 2, 0, 4, 0, 6, 0, 8, 9, 0, 0, 12, 13 };
...
static uint32_t speed_val, area_divider_val, mass_per_turn_val, time_min_val, time_sec_val, counted_mass_val, actual_mass_val, deviation_val, new_mass_val;
static uint32_t* val_ptrs[CCM_ELEMENT_VAL_COUNT] = { &speed_val, &area_divider_val, &mass_per_turn_val, &time_min_val, &time_sec_val, &counted_mass_val, &actual_mass_val, &deviation_val, &new_mass_val };
...
static const uint8_t speed_val_id = 21, area_divider_val_id = 22, mass_per_turn_val_id = 23, time_min_val_id = 24, time_sec_val_id = 25, counted_mass_val_id = 26, actual_mass_val_id = 27, deviation_val_id = 28, new_mass_val_id = 29;
static uint8_t val_ids[CCM_ELEMENT_VAL_COUNT] = { speed_val_id, area_divider_val_id, mass_per_turn_val_id, time_min_val_id, time_sec_val_id, counted_mass_val_id, actual_mass_val_id, deviation_val_id, new_mass_val_id };
...
static float CCMHelper_CalculateMotorSpeed(void);
...
static void CCMHelper_ConvertValToText(UI_Screen* screen, uint8_t val_pos)
{
	...
	{
		uint8_t length_before_dot = 3;
		if (val_pos == CCM_POS_DEVIATION_VAL)
		{
			length_before_dot = 2;
		}
	
		utils_val_to_text_converter(final_string, length, length_before_dot, value, type, selected_slot, selected_slot_gone);
	}
...
static float CCMHelper_CalculateMotorSpeed()
{
	if (area_divider_val != 0 && initial_time_total != 0 && mass_per_turn_val != 0 )
	{
		return ( 10000.0 / (float)area_divider_val * (float)user_quota / (float)mass_per_turn_val / ((float)initial_time_total / 60000) );
	}
	return 0;
}
...
static void CalibrationCheckMenu_ScreenCallback(UI_Screen* screen)
{
	...
	if (can_procedure_in_progress)
	{
		...
		counted_mass_val = round( (current_can_motor_speed * (float)total_motor_movement_time / 60000) * (float)mass_per_turn_val );
	...
	switch(press_type)
	{
		...
		case PRESS_TYPE_OK:
			switch (element_id)
			{
				...
				case begin_check_id:
				{
					current_can_motor_speed = CCMHelper_CalculateMotorSpeed();
				...
				{
					new_mass_val = round( (float)actual_mass_val / ( current_can_motor_speed * (float)total_motor_movement_time / 60000 ) );
					deviation_val = round( (float)abs((int32_t)new_mass_val - (int32_t)mass_per_turn_val) * 100 / (float)mass_per_turn_val * 100 );
		...
		case speed_item_id: case area_item_id: case time_item_id: case counted_mass_item_id: case actual_mass_item_id: case deviation_item_id: case new_mass_item_id:
		{
			...
			else
			{
				...
				switch (element_id)
				{
					...
					{
						initial_time_total = 0;
						
						if (speed_val != 0 && area_divider_val != 0 && user_seeder_width != 0)
						{
							initial_time_total = round( (10000.0 / (float)area_divider_val / ((float)speed_val * 3.6 * (float)user_seeder_width)) * 1000 );
						}
						time_min_val = initial_time_total / 60000;
						time_sec_val = (initial_time_total - time_min_val * 60000) / 1000;
						CCMHelper_ConvertValToText(screen, CCM_POS_TIME_MIN_VAL);
						CCMHelper_ConvertValToText(screen, CCM_POS_TIME_SEC_VAL);
						...
						{
							counted_mass_val = round( (float)user_quota * 10000 / (float)area_divider_val );
							CCMHelper_ConvertValToText(screen, CCM_POS_COUNTED_MASS_VAL);
							
							motor_speed_check = round( CCMHelper_CalculateMotorSpeed() * 10 );
						}
						
						if (motor_speed_check > 0 && motor_speed_check < 2560)
						{
							CCMHelper_SetElementFunctionality(screen, CCM_POS_BEGIN_CHECK, 1, 1);
						}
						else
						{
							CCMHelper_SetElementFunctionality(screen, CCM_POS_BEGIN_CHECK, 0, 1);
						}
...
void UI_BuildCalibrationCheckMenu(UI_Screen* screen)
{
  ...
  for (uint8_t i = 0; i < CCM_ELEMENT_TEXT_COUNT; i++)
  {
    ...
    // ---------------- Interactable ----------------
		...
		else if (label_ids[i] == speed_item_id || label_ids[i] == area_item_id)
		{
			if (user_max_speed != 0 && user_quota != 0 && user_seeder_width != 0 && user_mass_per_turn != 0)
			{
				CCMHelper_SetElementFunctionality(screen, i, 1, 1);
			}
			else
  ...
  for (uint8_t i = 0; i < CCM_ELEMENT_VAL_COUNT; i++)
  {
	...
	if (speed_val != 0 && area_divider_val != 0 && mass_per_turn_val != 0 && user_quota != 0 && user_seeder_width != 0)
	{
		...
		uint32_t motor_speed_check = round( CCMHelper_CalculateMotorSpeed() * 10 );
		if (motor_speed_check > 0 && motor_speed_check < 2560)
		{
			CCMHelper_SetElementFunctionality(screen, CCM_POS_BEGIN_CHECK, 1, 1);
		}
